...
lp.CharacterAdded:Connect(function()
    ...
    hookDash()
    rebuildPSet()
    -- clear cached remote on respawn so it re-searches in the new character
    cachedKLC = nil
end)
...
local function doSilentAim()
    ...
    for _, obj in ipairs(getDesc()) do
        if obj and obj:IsA("Humanoid") and obj.Health > 0
            and obj.Parent and obj.Parent ~= char and not pSet[obj.Parent] then
            check(obj.Parent)
        end
    end
    if best then
...
-- ────────────────────────── KITSUNE ATTACK (only) ──────────────────────────
-- cachedKLC declared before CharacterAdded so the reset above can reach it
local cachedKLC = nil
...
-- ────────────────────────── SNIFFER (hash capture) ──────────────────────────
-- Delta: wrap the whole __namecall hook in a pcall; if the sandbox rejects it
-- the script continues to work, just without the hash sniffer.
local sHash = "1169b354"
pcall(function()
    if not (_grm and _sro and _gnm) then return end
    local mt  = _grm(game)
    if not mt then return end
